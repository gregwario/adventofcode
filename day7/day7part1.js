...
given that this problem is based on a tree structure, i hypothesized
that the solution could be found in a logarithmic runtime.  whatever 
method is used, it would only be used a small number of times, so...
i used a cyclically inefficient method of control-f'ing my way to the 
answer.

starting with the first left-hand side element i came across, i 
literally control-f'ed each left-hand element all the way to the last 
element that has child elements.  this is the topmost element in the 
tower. 

kpjxln -> miauii -> hbmxey -> lqirhg -> hlhomy

5 control-f operations.  0 time spent writing code :)

(part 2 appears to be much more complicated, so i should probably 
start writing code for part 1 if i plan to solve part 2!)

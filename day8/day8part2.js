let instructions = `ioe dec 890 if qk > -10
gif inc -533 if qt <= 7
itw dec 894 if t != 0
...
ih dec 369 if ih == 1993`.split("\n");

let registers = { maxValue: 0 };
const parseInstruction = instruction => {
  ...
  if (registers[compareRegister] === undefined) {
    registers[compareRegister] = 0;
  }
  switch (comparator) {
    case "==":
      if (registers[compareRegister] == parseInt(compareValue)) {
        comparison = true;
      }
      break;
    case "!=":
    ...
    case "<=":
      ...
      break;
    default:
      console.log("uncaught comparison: ", comparator);
  }
  if (comparison) {
    if (direction === "inc") {
      registers[register] += parseInt(quantity, 10);
    } else if (direction === "dec") {
      registers[register] -= parseInt(quantity, 10);
    }
  }
  if (registers[register] > registers.maxValue) {
    registers.maxValue = registers[register];
  }
};
instructions.forEach(parseInstruction);
console.log("max value: ", registers.maxValue);

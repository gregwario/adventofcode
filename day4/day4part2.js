...
// same thing but we'll use a set
const isValidPassphrase = (phrase) => {
    return phrase.split(" ").reduce((acc, next) =>  {
        let sortedWord = next.split("").sort().join("");
        if (acc.wordlist.has(sortedWord)) {
            acc.hasDuplicateWord = true;
        } else {
            acc.wordlist.add(sortedWord);
        }
        return acc;
    }, {wordlist: new Set(), hasDuplicateWord: false})
    .hasDuplicateWord == false;
}

// some tests along the way...
// const validTest = `kebx eild nrskdr meja jxczomh gcne`;
// const invalidTest = `kebx xbek eild nrskdr meja jxczomh gcne`;
// isValidPassphrase(validTest);
// isValidPassphrase(invalidTest);

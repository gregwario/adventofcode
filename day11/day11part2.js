...
\          _____         _____
          /     \       /     \       
   \_____/ 0,0,1 \_____/ 0,-2,0\_____/
   /     \       /     \       /     \       
__/ 1,0,0 \_____/0,-1,0 \_____/-1,-2,0\_____/
  \       /     \       /     \       /     \
   \_____/ 0,0,0 \_____/-1,-1,0\_____/-2,-2,0\__
   /     \       /     \       /     \       /
__/ 0,1,0 \_____/-1,0,0 \_____/-2,-1,0\_____/
  \       /     \0,-1,-1/     \       /     \
   \_____/ 0,0,-1\_____/ -2,0,0\_____/-3,-1,0\__
   /     \       /     \       /     \       /
__/ 0,0,-2\_____/-1,0,-1\_____/ -3,0,0\_____/
  \       /     \0,-1,-2/     \       /     \
         / 0,0,-2\_____/-2,0,-1\_____/
/
s-direction

//   each compass direction maps to:
//   n: { r: 0, s: 0, t: 1 },
//   ne: { r: 0, s: -1, t: 0 },
//   se: { r: -1, s: 0, t: 0 },
//   s: { r: 0, s: 0, t: -1 },
...
iff coords are normalized, distance from origin: 
Math.Abs(r) + Math.Abs(s) + Math.Abs(t)

normalized means the coordinate has been reduced to fewer values if possible
...
const cartesianToHex = direction => {
  const hex = {
    r: 0,
    s: 0,
    t: 0
...
// testing that cartesianToHex works:
// console.log(
//   `cartesianToHex("ne")) == {r: 0, s: -1, t: 0}: ${JSON.stringify(
//     cartesianToHex("ne")
//   ) == '{"r":0,"s":-1,"t":0}'}`
// );

// console.log(
//   `cartesianToHex("s")) == {r: 0, s: 0, t: -1}: ${JSON.stringify(
//     cartesianToHex("s")
//   ) == '{"r":0,"s":0,"t":-1}'}`
// );

const addDirection = direction => {
  let { r, s, t } = cartesianToHex(direction);
  currentPosition.r += r;
  currentPosition.s += s;
  currentPosition.t += t;
};
// console.log(currentPosition) // {r: 0, s: 0, t: 0}
addDirection("n");
// console.log(currentPosition) // {r: 0, s: 0, t: 1}
const normalizeCoords = () => {
  // this function normalizes extra steps when possible, into shorter t steps
  // 1. (1, -1, 0) is the same as (0, 0, -1)
  // 2. (-1, 1, 0) is the same as (0, 0, 1)
  // 3. (0, -1, -1) is the same as (-1, 0, 0)
  // 4. (0, -1, 1) is the same as (0, 0, -2)
  // 5. (1, -1, -1) is the same as (0, 0, 0)
  // 6. (1, 1, -1) is the same as (0, 0, -2)
  // 7. (1, 0, -1) is the same as (0, 1, 0)
  // 8. (-1, 0, 1) is the same as (0, -1, 0)
  //   while (
  //     currentPosition.r !== 0 &&
  //     currentPosition.s !== 0 ||
  //     currentPosition.t !== 0
  //   ) {
  //     if (currentPosition.r > 0 && currentPosition.s < 0) {
  //       currentPosition.r--;
  //       currentPosition.s++;
  //       currentPosition.t--;
  //     }
  ...
  //       currentPosition.t++;
  //     }
  //     if (currentPosition.s < 0 && currentPosition.t < 0) {
  //       currentPosition.r--;
  //       currentPosition.s++;
  //       currentPosition.t++;
  //     }
...
const getDistance = position => {
  normalizeCoords();
  return Math.abs(position.r) + Math.abs(position.s) + Math.abs(position.t);
};

let furthestDistance = 0;
const traverseTheMap = () => {
...
// const mapped = mapTotalsToCoords(reduced);
// console.log(`mapped`, mapped);

// now how do we get back to origin from mapped
// luckily in this case, we can just add them together?
// 211 + 562 = 773
// if we had needed to step north or south, we would have
// to think about reducing the steps in that direction.
// so... this isn't a very good solution, but i can live
// with it for AoC. 😬

// console.log(`steps`, mapped.r + mapped.s);

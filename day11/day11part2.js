...
read this again, and decided to switch to a 3-axis grid system, since i need to reliably keep track of furthest distance.
http://www-cs-students.stanford.edu/~amitp/Articles/Hexagon2.html

here's how i'm defining my axes:
...
__/ 1,0,0 \_____/0,-1,0 \_____/-1,-2,0\_____/
  \       /xxxxx\       /     \       /     \
   \_____/x0,0,0x\_____/-1,-1,0\_____/-2,-2,0\__
   /     \xxxxxxx/     \       /     \       /
__/ 0,1,0 \xxxxx/-1,0,0 \_____/-2,-1,0\_____/
  \       /     \0,-1,-1/     \       /     \
...
iff coordinates have been reduced:
distance from origin = Math.Abs(r) + Math.Abs(s) + Math.Abs(t)
...
const reduce = () => {
  ...
  currentPosition = { r, s, t };
};

// // testing reduce logic
// console.log(
//   `reduce({ r: 2, s: -2, t: 0 })`,
//   reduce({ r: 2, s: -2, t: 0 })
// ); // { r: 0, s: 0, t: -2 }
...
const getDistance = position => {
  let { r, s, t } = currentPosition;
  return Math.abs(r) + Math.abs(s) + Math.abs(t);
};
